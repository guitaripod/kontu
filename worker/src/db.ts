...
export async function upsertListing(db: D1Database, n: NormalizedListing): Promise<UpsertResult> {
  ...
  await db
    .prepare(
      "UPDATE listings SET property_id = ?, url = ?, country = ?, property_type = ?, holding_form = COALESCE(?, holding_form), kiinteistotunnus = ?, " +
        ...
        "room_count = ?, room_layout = ?, floors = ?, year_built = COALESCE(?, year_built), occupancy_year = ?, " +
        ...
        "e_value = ?, risk_structures = ?, plot_ownership = COALESCE(?, plot_ownership), lease_end_year = ?, shore = COALESCE(?, shore), shore_sauna = COALESCE(?, shore_sauna), " +
